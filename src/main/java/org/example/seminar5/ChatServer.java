  ...
  private static class ClientHandler implements Runnable {
    ...
    public void run() {
      ...
      while (true) {
        ...
        if (type.equals(TypeMessage.MESSAGE)) {
        ...
        } else if (type.equals(TypeMessage.BROADCAST_MESSAGE)) {
          ...
          for (Map.Entry<String, ClientHandler>  entry : clients.entrySet()) {
            ...
            if (loginRecipients != this.clientLogin) {
              try {
                clientTo.sendMessage(msgBroadcast, TypeMessage.MESSAGE, loginSender);
    ...
    public void sendMessage(String message, TypeMessage type, String recipientLogin) throws JsonProcessingException {
      if (type == null) {
        out.println(message);
      } else if (type.equals(TypeMessage.MESSAGE)) {
        SendMessageRequest sendMessageResponse = new SendMessageRequest(TypeMessage.MESSAGE);
        sendMessageResponse.setRecipient(recipientLogin);
        sendMessageResponse.setMessage(message);
        out.println(sendMessageResponse);
      }
    }
